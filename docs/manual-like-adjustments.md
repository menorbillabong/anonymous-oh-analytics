...
- As métricas originais nunca são substituídas no banco. Painel e CG calculado identificam os extras. Relatórios, classificação e histórico mantêm os valores reais. Novo período inicia sem ajuste.
- TXT e CSV do período aberto incluem o ajuste salvo quando autorizado e ativado, usando a mesma distribuição determinística do Sheets. O total é aplicado uma única vez sobre todas as páginas e missões; baixar novamente não acumula. Desativado, revogado ou zerado: somente curtidas reais. Mantêm cinco colunas, tabulações do TXT e agrupamento por missão. Com extras, o cabeçalho é `Curtidas (X + manual)`, sem notas nas células. Configuração, permissão, sessão e período são revalidados antes do download; falhas interrompem a exportação em vez de gerar totais incompletos.
...
- Exportação: testes de TXT/CSV com 150 extras exatos, correspondência por publicação com o plano do Sheets, mais de 500 registros, duplicatas, isolamento de usuário, desativação, revogação, falhas e mudanças durante o download. Suíte de 96 testes e build de produção aprovados. Consulta somente leitura confirmou o contrato do RPC sem acesso; nenhum post ou ajuste real foi alterado.
...
Para reverter somente a inclusão do ajuste no TXT/CSV, retornar a versão de aplicação `9be86d9` ou reverter o commit dessa exportação. Não há migração de banco nesta mudança.
