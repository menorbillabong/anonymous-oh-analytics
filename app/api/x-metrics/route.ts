...
function mediaUrl(value:any){return typeof value==='string'?value:typeof value?.url==='string'?value.url:null}
function bestVideo(media:any){
 ...
 const thumbnail_url=mediaUrl(v.thumbnail_url)||mediaUrl(v.thumbnail)||mediaUrl(v.poster_url)||mediaUrl(v.poster)||mediaUrl(media?.thumbnail_url)||mediaUrl(media?.thumbnail)||null;
...
async function fetchPublicPost(handle:string,id:string){
 ...
 const text=String(t.text||'Publicação do X').trim()||'Publicação do X';
 const views=Number(t.views||0),likes=Number(t.likes||0),reposts=Number(t.retweets??t.reposts??0),comments=Number(t.replies||0);
 return {title:text,text,views,likes,reposts,comments,metrics:{views,likes,reposts,comments},published_date:created&&!Number.isNaN(created.getTime())?created.toISOString().slice(0,10):null,published_at:created&&!Number.isNaN(created.getTime())?created.toISOString():null,image_urls:(t.media?.photos||[]).map((x:any)=>x?.url).filter(Boolean),video_url:video.video_url,thumbnail_url:video.thumbnail_url,author_handle:t.author?.screen_name||handle,author_name:t.author?.name||t.author?.display_name||t.author?.screen_name||handle,author_avatar:t.author?.avatar_url||t.author?.avatar||t.author?.profile_image_url||null,source:'auto'};
}
