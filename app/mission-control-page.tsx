...
type Mission={id:string|number;name:string;description:string;multiplier:number;reward:number;submission_limit:number;color:string;active:boolean};
...
export default function MissionControlPage({uid,reloadProfiles}:{uid:string;reloadProfiles:()=>Promise<void>}){
 const[rows,setRows]=useState<Mission[]>([]),[name,setName]=useState(defaults.name),[description,setDescription]=useState(defaults.description),[multiplier,setMultiplier]=useState(defaults.multiplier),[reward,setReward]=useState(defaults.reward),[submissionLimit,setSubmissionLimit]=useState(defaults.submissionLimit),[color,setColor]=useState(defaults.color);
 ...
 const load=useCallback(async()=>{const{data,error}=await supabase.from('mission_profiles').select('*').eq('user_id',uid).order('active',{ascending:false}).order('name');if(error){setNotice('Não foi possível carregar as missões.');return}setRows((data||[]) as Mission[])},[uid]);
 useEffect(()=>{load()},[load]);

 function resetForm(){setName(defaults.name);setDescription(defaults.description);setMultiplier(defaults.multiplier);setReward(defaults.reward);setSubmissionLimit(defaults.submissionLimit);setColor(defaults.color);setEditing(null)}
 function startEdit(mission:Mission){setEditing(mission);setName(mission.name||'');setDescription(mission.description||'');setMultiplier(Number(mission.multiplier||defaults.multiplier));setReward(Number(mission.reward||0));setSubmissionLimit(Number(mission.submission_limit||0));setColor(mission.color||defaults.color);setNotice('');window.scrollTo({top:0,behavior:'smooth'})}

 async function save(){
  const cleanName=name.trim();if(!cleanName||busy||multiplier<=0||reward<0||submissionLimit<0)return;
  setBusy(true);setNotice('');const missionData={name:cleanName,description:description.trim(),multiplier:Number(multiplier),reward:Number(reward),submission_limit:Number(submissionLimit),color};
 ...
 return <section className="mission-control-reference">
  ...
  <div className="mission-control-existing"><h2>Missões existentes</h2><div className="mission-existing-grid">{rows.map(mission=><article key={mission.id} className="mission-existing-card" style={{'--mission-color':mission.color||defaults.color} as React.CSSProperties}>
   <div className="mission-existing-main"><h3>{String(mission.name||'MISSÃO').toUpperCase()}</h3><p>{mission.description||'Sem descrição.'}</p><div className="mission-tags"><span>{Number(mission.multiplier||2).toLocaleString('pt-BR',{maximumFractionDigits:2})}X</span><span>BÔNUS {Number(mission.reward||0).toLocaleString('pt-BR')}</span><span>{mission.submission_limit>0?`${mission.submission_limit} MÁX.`:'ILIMITADO'}</span></div></div>
