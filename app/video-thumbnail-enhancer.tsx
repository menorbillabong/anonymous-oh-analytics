...
function bind(video:HTMLVideoElement,postUrl?:string,knownPoster?:string){
 if(video.dataset.aohPreviewManaged==='react'||video.dataset.aohThumbBound==='1'||!postUrl||!postPattern.test(postUrl))return;
...
export default function VideoThumbnailEnhancer(){
 useEffect(()=>{let alive=true;const map=new Map<string,string>();let addTimer:number|undefined;
  ...
  const relevant='video:not([data-aoh-preview-managed="react"]),.add-publication-screen';
  let scanFrame=0;
  const observer=new MutationObserver(records=>{
   const hasNewMedia=records.some(record=>Array.from(record.addedNodes).some(node=>node instanceof Element&&(node.matches(relevant)||node.querySelector(relevant))));
   if(!hasNewMedia||scanFrame)return;
   scanFrame=requestAnimationFrame(()=>{scanFrame=0;if(alive)scan()});
  });
  supabase.auth.getSession().then(async({data})=>{const uid=data.session?.user.id;if(!uid||!alive)return;const{data:rows}=await supabase.from('posts').select('post_url,video_url').eq('user_id',uid);if(!alive)return;for(const row of rows||[]){if(row.video_url&&row.post_url)map.set(String(row.video_url),String(row.post_url))}scan()});scan();observer.observe(document.body,{childList:true,subtree:true});return()=>{alive=false;window.clearTimeout(addTimer);cancelAnimationFrame(scanFrame);observer.disconnect()}
 },[]);return null;
}
