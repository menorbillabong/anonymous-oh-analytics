...
export default function MissionSelectionPage({uid,onClose,onSaved}:{uid:string;onClose:()=>void;onSaved:()=>Promise<void>}){
 ...
 const load=useCallback(async(synchronize=true)=>{
  if(synchronize){const {error}=await supabase.rpc('sync_my_mission_period_selections');if(error)throw error;}
 ...
 async function toggle(post:any,assigned:boolean){
  ...
  const existingBonus=profiles.find(p=>String(p.id)===String(post.mission_profile_id)&&Number(p.reward)>0);
  try{const {error}=await supabase.rpc('set_my_mission_period_selection',{p_period:period.id,p_post:post.id,p_profile:assigned?null:Number(existingBonus?.id||chosen)});if(error)throw error;await load(false);await onSaved();setNotice(assigned?'Publicação desmarcada. Você pode selecionar outra no lugar.':'Publicação selecionada e indisponível nos outros períodos.');}
  catch(error){setNotice(missionSelectionError(error as {message?:string}));try{await load(false);}catch{setFailed(true);}}
 ...
 {!loading&&!failed&&period&&<>
 <div className="mission-period-toolbar"><div className="mission-period-fields">
  <label>Período<select value={selectedPeriod} disabled={busy} onChange={e=>{setSelectedPeriod(e.target.value);setNotice('');}}>{periods.map(p=><option key={p.id} value={p.id}>{formatPostDate(p.start_date)} a {formatPostDate(p.end_date)}</option>)}</select></label>
  <div className="mission-period-counter"><span>Publicações selecionadas no período</span><strong className="mission-period-count" role="status" aria-live="polite" aria-atomic="true">{count} selecionadas · Seleção manual livre</strong><small>{period.per_user_limit===0?'Preenchimento automático desativado':period.per_user_limit===null?'Preenchimento automático aguardando definição do administrador':`Referência para preenchimento automático: ${period.per_user_limit} por usuário`}</small></div>
 </div></div>
 <aside className="mission-period-floating" aria-label="Seleção de missão"><label htmlFor="mission-floating-profile">Perfil para as novas seleções</label><select id="mission-floating-profile" value={chosen} disabled={busy||!bonusProfiles.length} onChange={e=>setProfileId(e.target.value)}>{!bonusProfiles.length&&<option value="">Nenhum perfil ativo com bônus</option>}{bonusProfiles.map(p=><option key={p.id} value={p.id}>{p.name} (+{p.reward} CG)</option>)}</select><small>A missão escolhida será usada nas próximas seleções.</small></aside>
 <div className="mission-period-summary"><span>Datas inclusivas · suas publicações elegíveis</span></div>{!bonusProfiles.length&&<p>Crie um perfil com bônus fixo no Centro de Controle da Missão para selecionar publicações.</p>}
 <div className={`mission-review-grid ${mode}`}>{visible.map(post=>{const assignment=selections.find(row=>String(row.post_id)===String(post.id)),assigned=Boolean(assignment),existingBonus=profiles.find(p=>String(p.id)===String(post.mission_profile_id)&&Number(p.reward)>0);return <article className="mission-review-card" key={post.id}><div className="mission-review-preview"><XPostPreview post={post}/></div><div className={`mission-period-choice${assigned?' selected':''}`}><strong>{assigned?`Selecionada • ${profiles.find(p=>String(p.id)===String(assignment?.mission_profile_id))?.name||post.mission_name||'Missão com bônus'}`:existingBonus?`Já possui bônus • ${existingBonus.name} • aguardando vínculo`:'Disponível para missão'}</strong><button type="button" disabled={busy||Boolean(post.counting_excluded)||(!assigned&&!existingBonus&&!chosen)} onClick={()=>void toggle(post,assigned)}>{post.counting_excluded?'Período fechado':assigned?'Desmarcar publicação':existingBonus?'Vincular ao período (manter perfil)':'Selecionar para missão'}</button></div></article>;})}</div>{!visible.length&&<p className="mission-period-empty">Nenhuma publicação elegível neste período.</p>}<p className="mission-period-empty">Cada publicação fica em apenas um período. Você pode desmarcar uma seleção para corrigir seu perfil e selecionar outra. O preenchimento automático não limita suas seleções manuais.</p></>}
