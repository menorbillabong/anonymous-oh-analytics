...
export default function Dashboard({ session }: {
    ...
    const load = useCallback(async (pulse = false) => { const before = totalsFor(posts); if (pulse) {
        ...
        setRefreshStep(2); const next = p || [], nextSettings = { ...settingsDefaults, ...(s || {}), ...normalizeMatrixAppearance(s) }; setPosts(next); setSettings(nextSettings); setXImportAccess({ enabled: Boolean(xAccess?.enabled), handle: String(xAccess?.handle || s?.x_handle || '').replace(/^@/, '') }); setActivePeriod(period?.id && period?.start_date ? period as ActivePeriod : null); setProfileChecked(true); setProfiles(m || []); void syncRanking(); if (pulse) {
...
function PanelActions({organized,activePeriod,xImportEnabled,xHandle,refreshing,busy,monthlyPostRemaining,monthlyPostGoal,userId,openPeriod,closePeriod,searchX,onXHandleSaved,openBulk,refreshMetrics,openAdd}:{organized:boolean;activePeriod:ActivePeriod|null;xImportEnabled:boolean;xHandle:string;refreshing:boolean;busy:boolean;monthlyPostRemaining:number;monthlyPostGoal:number;userId:string;openPeriod:()=>void;closePeriod:()=>void;searchX:()=>void;onXHandleSaved:(handle:string)=>void;openBulk:()=>void;refreshMetrics:()=>Promise<boolean>;openAdd:()=>void}) {
    ...
    const publications = <><XImportControls userId={userId} enabled={xImportEnabled} handle={xHandle} busy={busy} atMonthlyLimit={monthlyPostRemaining <= 0} onSearch={searchX} onHandleSaved={onXHandleSaved}/><button className="orange-add" disabled={monthlyPostRemaining <= 0} title={monthlyPostRemaining <= 0 ? 'Meta mensal atingida' : `${monthlyPostRemaining} vaga(s) restante(s) neste mês`} onClick={openBulk}>▤ <b>ADIÇÃO EM MASSA</b></button><button className="orange-add" disabled={monthlyPostRemaining <= 0} title={monthlyPostRemaining <= 0 ? 'Meta mensal atingida' : `${monthlyPostRemaining} vaga(s) restante(s) neste mês`} onClick={openAdd}>＋ <b>{monthlyPostRemaining <= 0 ? 'META ATINGIDA' : 'ADICIONAR PUBLICAÇÃO'}</b></button></>;
    const files = <><PostExportButtons userId={userId}/><button type="button" className="blue-btn mission-period-button" onClick={() => window.dispatchEvent(new Event('aoh:open-mission-periods'))}>▦ <b>PERÍODOS DE MISSÃO</b></button><button className="report-btn">▤ <b>RELATÓRIO</b></button></>;
    const updates = <><button className={`green-btn metrics-refresh-button ${refreshing ? 'is-refreshing' : ''}`} disabled={refreshing} onClick={() => void refreshMetrics()}>↻ <b>{refreshing ? 'ATUALIZANDO MÉTRICAS...' : 'ATUALIZAR MÉTRICAS'}</b></button><GoogleSheetsSyncButton userId={userId} beforeSync={refreshMetrics}/></>;
