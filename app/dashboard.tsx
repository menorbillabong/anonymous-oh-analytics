...
import {MatrixBackground, useMatrixAppearance} from './matrix-background';
import {normalizeMatrixAppearance} from '@/lib/matrix-appearance';
...
import XImportControls from './x-import-controls';
import PostExportButtons from './post-export-buttons';
...
import {useEdgeAppearance} from './edge-appearance';
import {PwaInstall} from './pwa';
import {normalizeEdgeAppearance} from '@/lib/edge-appearance';
...
export default function Dashboard({ session }: {
    ...
    useMatrixAppearance(settings, uid);
    useEdgeAppearance(settings, uid);
    ...
    const [xImportAccess, setXImportAccess] = useState<{ enabled: boolean; handle: string }>({ enabled: false, handle: '' });
    ...
    const requireOpenPeriod = () => { if (activePeriod)
        return true; setRefreshNotice('Abra um período antes de adicionar ou buscar publicações.'); setOpenPeriodOpen(true); return false; };
    const openAdd = () => { if (!requireOpenPeriod())
    ...
    const load = useCallback(async (pulse = false) => { const before = totalsFor(posts); if (pulse) {
        ...
        setRefreshStep(2); const next = p || [], nextSettings = { ...settingsDefaults, ...(s || {}), ...normalizeMatrixAppearance(s), ...normalizeEdgeAppearance(s) }; setPosts(next); setSettings(nextSettings); setXImportAccess({ enabled: Boolean(xAccess?.enabled), handle: String(xAccess?.handle || s?.x_handle || '').replace(/^@/, '') }); setActivePeriod(period?.id && period?.start_date ? period as ActivePeriod : null); setProfileChecked(true); setProfiles(m || []); void syncRanking(); if (pulse) {
    ...
    useEffect(() => { const close = (e: KeyboardEvent) => { if (e.key !== 'Escape')
        ...
        setPeriodOpen(false); if (openPeriodOpen)
        setOpenPeriodOpen(false); }; window.addEventListener('keydown', close); return () => window.removeEventListener('keydown', close); }, [addOpen, periodOpen, openPeriodOpen]);
    ...
    async function processBulk() { if (!activePeriod) {
        ...
        setOpenPeriodOpen(true);
        return;
    } const urls = [...new Set((bulkText.match(/https?:\/\/[^\s]+/g) || []).map(x => x.replace(/[),.;]+$/, '')))].filter(x => /https?:\/\/(www\.)?(x\.com|twitter\.com)\//i.test(x)); if (!urls.length) {
    ...
    if (missionPeriodsOpen)
        return <div className="exact-app"><MatrixBackground/><main className="exact-main"><MissionSelectionPage uid={uid} onClose={() => setMissionPeriodsOpen(false)} onSaved={() => load(false)}/></main></div>;
    if (reportOpen)
        return <ReportPage posts={countedPosts} profiles={profiles} settings={settings} onCancel={() => setReportOpen(false)}/>;
    return <div className="exact-app"><MatrixBackground/>
    <header className="exact-topbar"><div className="exact-brand"><span>AVALIAÇÃO&nbsp;DE&nbsp;MÉTRICAS</span><PwaInstall/></div>{refreshing ? <div className="refresh-pill"><span className="refresh-spin">↻</span><span>ATUALIZANDO MÉTRICAS<br /><b>({refreshStep}/2)</b></span></div> : <div className="wait-pill">●&nbsp; ESPERA</div>}<nav className="exact-nav">{[...nav, ...(isAdmin ? ['Admin'] : [])].map(item => <button key={item} className={tab === item ? 'active' : ''} onClick={() => chooseTab(item)}><span className="nav-icon">{iconFor(item)}</span><span>{item === 'História' ? 'Histórico' : item === 'Centro de Controle da Missão' ? 'Missões' : item === 'Registro de atividades' ? 'Atividades' : item}</span></button>)}</nav><button className="user-pill" onClick={signOut}><span>◉</span>{username}</button></header>
    ...
    <OpenPeriodModal
      posts={countedPosts}
      period={activePeriod}
      onClose={() => setOpenPeriodOpen(false)}
      onSuccess={async () => {
        setOpenPeriodOpen(false);
        await load(false);
        setRefreshNotice(activePeriod ? 'Data inicial corrigida com segurança.' : 'Período aberto. Agora você pode adicionar e buscar publicações.');
      }}/>
    )}
    {periodOpen && activePeriod && (
    <ClosePeriodModal
      posts={countedPosts}
      periodStart={activePeriod.start_date}
      closeAvailableOn={activePeriod.close_available_on}
      canClose={activePeriod.can_close !== false}
      earlyReleaseSource={activePeriod.early_release_source}
      crystalginLimit={crystalginLimit}
      onClose={() => setPeriodOpen(false)}
      onSuccess={async () => {
        setPeriodOpen(false);
        await load(false);
        setRefreshNotice('Período fechado. As publicações foram retiradas das contagens atuais.');
        chooseTab('História');
      }}/>
    )}
...
function PanelActions({organized,activePeriod,xImportEnabled,xHandle,refreshing,busy,monthlyPostRemaining,monthlyPostGoal,userId,openPeriod,closePeriod,searchX,onXHandleSaved,openBulk,refreshMetrics,openAdd}:{organized:boolean;activePeriod:ActivePeriod|null;xImportEnabled:boolean;xHandle:string;refreshing:boolean;busy:boolean;monthlyPostRemaining:number;monthlyPostGoal:number;userId:string;openPeriod:()=>void;closePeriod:()=>void;searchX:()=>void;onXHandleSaved:(handle:string)=>void;openBulk:()=>void;refreshMetrics:()=>Promise<boolean>;openAdd:()=>void}) {
    const period = <div className="period-action-pair"><button data-appearance-button="open" className="blue-btn" title={activePeriod ? `Corrigir o início definido em ${activePeriod.start_date}` : 'Definir a data inicial do período'} onClick={openPeriod}>{activePeriod ? '✎' : '▶'} <b>{activePeriod ? 'CORRIGIR INÍCIO' : 'ABRIR PERÍODO'}</b></button><button data-appearance-button="close" className="blue-btn" disabled={!activePeriod} title={!activePeriod ? 'Abra um período primeiro' : activePeriod.can_close === false && activePeriod.close_available_on ? `Fechamento normal a partir de ${activePeriod.close_available_on}` : `Fechar período iniciado em ${activePeriod.start_date}`} onClick={closePeriod}>◴ <b>FECHAR PERÍODO</b></button></div>;
    const publications = <><XImportControls userId={userId} enabled={xImportEnabled} handle={xHandle} busy={busy} atMonthlyLimit={monthlyPostRemaining <= 0} onSearch={searchX} onHandleSaved={onXHandleSaved}/><button data-appearance-button="bulk" className="orange-add" disabled={monthlyPostRemaining <= 0} title={monthlyPostRemaining <= 0 ? 'Meta mensal atingida' : `${monthlyPostRemaining} vaga(s) restante(s) neste mês`} onClick={openBulk}>▤ <b>ADIÇÃO EM MASSA</b></button><button data-appearance-button="add" className="orange-add" disabled={monthlyPostRemaining <= 0} title={monthlyPostRemaining <= 0 ? 'Meta mensal atingida' : `${monthlyPostRemaining} vaga(s) restante(s) neste mês`} onClick={openAdd}>＋ <b>{monthlyPostRemaining <= 0 ? 'META ATINGIDA' : 'ADICIONAR PUBLICAÇÃO'}</b></button></>;
    const files = <><PostExportButtons userId={userId}/><button type="button" data-appearance-button="missions" className="blue-btn mission-period-button" onClick={() => window.dispatchEvent(new Event('aoh:open-mission-periods'))}>▦ <b>PERÍODOS DE MISSÃO</b></button><button data-appearance-button="report" className="report-btn">▤ <b>RELATÓRIO</b></button></>;
    const updates = <><button data-appearance-button="metrics" className={`green-btn metrics-refresh-button ${refreshing ? 'is-refreshing' : ''}`} disabled={refreshing} onClick={() => void refreshMetrics()}>↻ <b>{refreshing ? 'ATUALIZANDO MÉTRICAS...' : 'ATUALIZAR MÉTRICAS'}</b></button><GoogleSheetsSyncButton userId={userId} beforeSync={refreshMetrics}/></>;
    if (!organized) return <div className="hero-actions">{period}{files}{publications}{updates}</div>;
    return <div className="hero-actions organized-actions"><div className="action-group"><small>PUBLICAÇÕES</small><div>{publications}</div></div><div className="action-group update-action-group"><small>ATUALIZAÇÃO E PLANILHA</small><div>{updates}</div></div><div className="action-group"><small>RELATÓRIOS E ARQUIVOS</small><div>{files}</div></div><div className="action-group period-group"><small>PERÍODO</small><div>{period}</div></div></div>;
}
