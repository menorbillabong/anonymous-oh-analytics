...
export default function DatabaseRefreshTimer(){
 ...
 useEffect(()=>{const manual=async(event:MouseEvent)=>{if(!uid||!state.active||firing.current)return;const button=(event.target as HTMLElement).closest('button');if(!button||(button.textContent||'').toUpperCase().indexOf('ATUALIZAR AGORA')<0)return;const now=new Date(),next=new Date(now.getTime()+msFor(state.hours));await supabase.from('user_settings').update({last_refresh_at:now.toISOString(),next_refresh_at:next.toISOString()}).eq('user_id',uid);setState(s=>({...s,next:next.toISOString()}))};document.addEventListener('click',manual,true);return()=>document.removeEventListener('click',manual,true)},[uid,state.active,state.hours]);
