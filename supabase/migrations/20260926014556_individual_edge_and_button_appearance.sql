-- Additive, per-account preferences. Keep all existing RLS/grants and data.
set lock_timeout = '5s';
set statement_timeout = '30s';
alter table public.user_settings
  add column border_glow_intensity smallint not null default 50
    constraint user_settings_border_glow_intensity_check check (border_glow_intensity between 0 and 100),
  add column button_colors jsonb not null default '{}'::jsonb
    constraint user_settings_button_colors_check check (
      jsonb_typeof(button_colors) = 'object'
      and octet_length(button_colors::text) <= 2048
      and button_colors - array['x','bulk','add','metrics','sheets','missions','report','open','close','txt','csv'] = '{}'::jsonb
      and not jsonb_path_exists(button_colors, '$.* ? (@.type() != "string" || !(@ like_regex "^#[0-9a-fA-F]{6}$"))')
    );
comment on column public.user_settings.border_glow_intensity is 'Personal border glow: 0 disables, 50 preserves the previous glow, 100 is strong.';
comment on column public.user_settings.button_colors is 'Personal dashboard button hex overrides. Empty object preserves default colors.';
...
begin
  ...
  for v_key, v_value in select key, value from jsonb_each(p_settings) loop
    if not (v_key = any(array['matrix_enabled','matrix_color','x_handle',
      ...
      'surface_color','border_color','language','monthly_post_goal','border_glow_intensity','button_colors'])) then continue; end if;
    v_text := v_value #>> '{}';
    if v_value = 'null'::jsonb then raise exception 'INVALID_PRESET'; end if;
    if v_key = 'border_glow_intensity' then
      if jsonb_typeof(v_value) <> 'number' or v_text::numeric not between 0 and 100 or trunc(v_text::numeric) <> v_text::numeric then raise exception 'INVALID_PRESET'; end if;
    elsif v_key = 'button_colors' then
      if jsonb_typeof(v_value) <> 'object' then raise exception 'INVALID_PRESET'; end if;
      -- The user_settings CHECK also validates identifiers and color values.
    elsif v_key = any(array['matrix_enabled','ranking_opt_in','show_refresh_timer','cap_unlocked']) then
  ...
  update public.user_settings set
    ...
    border_glow_intensity=v_settings.border_glow_intensity,button_colors=v_settings.button_colors,
