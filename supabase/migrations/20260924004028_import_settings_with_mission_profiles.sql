...
begin
  ...
  for v_key, v_value in select key, value from jsonb_each(p_settings) loop
    if not (v_key = any(array['matrix_enabled','matrix_color','x_handle',
      ...
      'cap_unlocked','crystalgin_limit','app_name','accent_color','background_color',
    ...
    if v_key = any(array['matrix_enabled','ranking_opt_in','show_refresh_timer','cap_unlocked']) then
      ...
      if v_key = 'app_name' then
        v_text := btrim(v_text);
        if length(v_text) not between 2 and 40 then raise exception 'INVALID_PRESET'; end if;
      elsif v_key = 'x_handle' then
  ...
  update public.user_settings set
    ...
    crystalgin_limit=v_settings.crystalgin_limit,app_name=v_settings.app_name,
    ...
    profile_name_confirmed=true,updated_at=now()
