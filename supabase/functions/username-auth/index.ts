...
Deno.serve(async(req:Request)=>{
 ...
 try{
  ...
  if(action==="admin-period-close-status"||action==="admin-reset-period-close"||action==="admin-release-all-period-close"){
   ...
   const releaseAll=action==="admin-release-all-period-close";
   const reason=String(rawReason??"").trim();
   if(reset&&!validUserId(targetUserId))return reply(origin,{error:"Conta de destino inválida."},400);
   if((reset||releaseAll)&&(reason.length<3||reason.length>500))return reply(origin,{error:"Informe um motivo entre 3 e 500 caracteres."},400);

   const{data:result,error:cooldownError}=await admin.rpc("admin_period_close_cooldown_verified",{
    p_actor:caller.user.id,
    p_action:reset?"reset":releaseAll?"release_all":"status",
    p_target_user:reset?String(targetUserId):null,
    p_reason:reset||releaseAll?reason:null,
   });
   if(cooldownError){
    ...
    return reply(origin,{error:reset||releaseAll?"Não foi possível liberar o fechamento. Nenhum período foi alterado.":"Não foi possível consultar os prazos de fechamento."},500);
   }
   return reset||releaseAll?reply(origin,{ok:true,release:result}):reply(origin,{ok:true,cooldowns:Array.isArray(result)?result:[]});
  }
